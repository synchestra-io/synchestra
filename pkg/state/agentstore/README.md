...
- `LeaseStore.Acquire` records `LeaseAcquireParams.TTL` onto `AuthorityLease.ExpiresAt` but no implementation evaluates it: a lease is held until an explicitly fenced `Release`, however long past its recorded expiry. Task-3's `abandoned-run-is-resumable` work owns deciding and implementing the expiry/reconciliation pass (evaluating `ExpiresAt`, reclaiming an abandoned run's lease, or both) that closes this gap; it is deliberately not implemented here.
- `pkg/state/gitstore`'s `Agent()` wiring (`buildAgentCore`) pins its `replication.Journal` to explicit batching-off (`MaxBatchItems`/`MaxBatchDelayMS` both zero) rather than the documented 100-item/1000ms default: no caller reaches it concurrently today, this package's `Store` has no `Close` path wired through `state.AgentStore`/`GitStateStore` for a batched journal to flush against on shutdown, and turning batching on measured an 11x regression in `pkg/state/gitstore`'s own test suite. Task-3 owns wiring a real `Close` path plus real concurrent callers, and should enable batching in the same change.
