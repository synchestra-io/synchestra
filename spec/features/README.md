...
| [project](project/README.md) | Draft | Canonical project data model and ACL — project ID as repo reference (`{provider}/{namespace}/{slug}`); yaml-authoritative membership at `.synchestra/project-members.yaml` with `(iss, sub, role)` federated identities; Firestore `projects/{id}` as a cache for cloud-mediated auth; host-local TTL cache for bypass-auth hosts |
