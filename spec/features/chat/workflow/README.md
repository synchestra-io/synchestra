...
Built-in workflow definitions ship with Synchestra. Project-level overrides and customizations live in `synchestra-spec-repo.yaml`.
...
Projects customize built-in workflows in `synchestra-spec-repo.yaml`. The built-in workflows define the structure (steps, order, artifact types); the project configures the substance (questions, rules, checks).
