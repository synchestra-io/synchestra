...
Adds one or more code repositories to the project. Each code repo is appended to the `repos` list in the spec repo's `synchestra-spec.yaml`. If a repo is not already on disk, it is cloned. A `synchestra-code.yaml` file is written to each newly added repo.
...
| `1` | Conflict — code repo already has a `synchestra-code.yaml` for a different project |
| `2` | Invalid arguments |
...
3. For each `--code-repo`:
   ...
   d. If `synchestra-code.yaml` exists and points to a different project, exit `1`
   e. Skip if already in the `repos` list
   f. Write `synchestra-code.yaml` with `spec_repos` pointing to the spec repo
