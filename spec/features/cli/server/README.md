...
This group deviates from the `<resource> <action>` pattern used by commands like `task claim`. The lifecycle subcommands (`start`, `stop`, `restart`, `status`) manage a daemon process, not a domain resource. The `project` sub-group nests project management under this namespace, creating three-level commands (e.g., `synchestra server project add`), which is justified by the tight coupling between project management and server configuration.
...
| [project](project/README.md) | Project management |
