...
| `--current` | For update | Expected current status (guard against stale state) |
| `--new` | For update | Target status to transition to |
| `--reason` | No | Reason for the transition (required for `failed` and `blocked`, optional otherwise) |
