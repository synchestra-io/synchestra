...
`KindUnknownTarget`, `KindNoInstallDir`, and `KindDestinationExists` are mapped explicitly even though `synchestra self-update`'s own `Config.Update`/`Config.Check` never produce them (`cliinstall`'s planner does, for [`synchestra install`](../install/README.md), which now exists and maps the same three kinds identically) — appended after `KindManagedCommand` in the shared library's `FailureKind` enum, so no existing mapping's meaning changes, and pinned by `pkg/cli/selfupdate/selfupdate_test.go`'s `TestErrorMapperFailureNewKinds` and, for `install` itself, by `pkg/cli/install/install_test.go`'s `TestErrorMapperFailureNewKinds`.
