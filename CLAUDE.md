...
- CLI arguments are documented in `$args/` directories under `spec/features/cli/`. Each argument has its own `.md` file at the level where it applies (global, command-group, or command-specific). See the [`$args` convention in the CLI spec](spec/features/cli/README.md#the-args-directory-convention) for the full format and placement rules.
